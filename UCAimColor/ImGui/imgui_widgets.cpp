...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size - 1), inner_rect.Min.y, outer_rect.Max.x - 1, inner_rect.Max.y);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.f)), 0.0f, 3.0f), -ImClamp(IM_FLOOR((bb_frame_height - 2.0f) * 0.5f), 0.f, 3.0f)));
    ...
    const ImU32 grab_col = GetColorU32(hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);
...
bool ImGui::Tab(const char* label, const char* icon, bool selected)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(85 * Menu::DPI_SCALE, 50 * Menu::DPI_SCALE));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Chec    ked : 0));
        return false;
    }
    ...
    float AnimationSelected = FastFloatLerp(std::string(label).append("AnimationSelected"), selected, 0, 1, 0.03f);
    ImColor ColorSelected = FastColorLerp(ImColor(255, 255, 255), Menu::MenuColor, AnimationSelected);

    static float line_pos = 0;
    line_pos = ImLerp(line_pos, selected ? total_bb.Min.y - window->Pos.y : line_pos, 0.1f);

    PushFont(render_menu::Icons);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 42.5f * Menu::DPI_SCALE - CalcTextSize(icon).x / 2, total_bb.Min.y + 25 * Menu::DPI_SCALE - CalcTextSize(icon).y / 2), ColorSelected, icon);
    PopFont();

    window->DrawList->AddRectFilled(ImVec2(total_bb.Min.x + 1 * Menu::DPI_SCALE, GetWindowPos().y + line_pos + 5 * Menu::DPI_SCALE), ImVec2(total_bb.Min.x + 5 * Menu::DPI_SCALE, GetWindowPos().y + line_pos + 45 * Menu::DPI_SCALE), Menu::MenuColor, 4 * Menu::DPI_SCALE + 1, ImDrawCornerFlags_Right);
...
bool ImGui::KeyBind(const char* label, int* k)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(window->Size.x - 15 - 5 - CalcTextSize(KeyNames[*k]).x - 5, 2), window->DC.CursorPos + ImVec2(window->Size.x - 15, 20 * Menu::DPI_SCALE));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(window->Size.x, 20 * Menu::DPI_SCALE));
    ...
    float animhv = FastFloatLerp(std::string(label).append("anif"), hovered, 0, 1, 0.04f);
    ImColor animationhover = FastColorLerp(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), ImColor(74, 74, 74, (int)(GetStyle().Alpha * 255)), animhv);
    ...
    float animtx = FastFloatLerp(std::string(label).append("anitx"), output || output2, 0, 1, 0.04f);
    ImColor animatiotx = FastColorLerp(ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), ImColor(186, 186, 186, (int)(GetStyle().Alpha * 255)), animtx);

    window->DrawList->AddText(ImVec2(total_bb.Min.x + 15, frame_bb.Min.y + 9 - CalcTextSize(label).y / 2), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), label);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(44, 44, 44, (int)(GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, animationhover, 3 * Menu::DPI_SCALE + 1);
    window->DrawList->AddText(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y + 9 - CalcTextSize(buf_display).y / 2), animatiotx, buf_display);
    ...
    ImGui::SetCursorPosY(GetCursorPosY() + 5 * Menu::DPI_SCALE);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 25 * Menu::DPI_SCALE;
    const ImVec2 pos = window->DC.CursorPos;
    ImRect total_bb;

    if (window->ScrollTarget.y == 0)
        total_bb = ImRect(pos, pos + ImVec2(window->Size.x, square_sz));
    else
        total_bb = ImRect(pos, pos + ImVec2(window->Size.x - window->ScrollbarSizes.x + 5 * Menu::DPI_SCALE, square_sz));
    ...
    window->DrawList->AddRectFilled(ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE - 25 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) - 5 * Menu::DPI_SCALE), ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) + 5 * Menu::DPI_SCALE), ImColor(44, 44, 44, (int)(ImGui::GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);

    float Animation = FastFloatLerp(std::string(label).append("anm"), *v, 2, 19 * Menu::DPI_SCALE, 1);
    window->DrawList->AddRectFilled(ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE - 25 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) - 4.5f * Menu::DPI_SCALE), ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE - 25 * Menu::DPI_SCALE + Animation + 5 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) + 4.5f * Menu::DPI_SCALE), ImColor(Menu::MenuColor.Value.x, Menu::MenuColor.Value.y, Menu::MenuColor.Value.z, ImGui::GetStyle().Alpha), 4 * Menu::DPI_SCALE + 1);

    float animhv = FastFloatLerp(std::string(label).append("anif"), hovered, 0, 1, 0.04f);
    ImColor animationhover = FastColorLerp(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), ImColor(74, 74, 74, (int)(GetStyle().Alpha * 255)), animhv);

    window->DrawList->AddRect(ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE - 26 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) - 5 * Menu::DPI_SCALE), ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) + 5 * Menu::DPI_SCALE), animationhover, 3 * Menu::DPI_SCALE + 1);
    window->DrawList->AddCircleFilled(ImVec2(total_bb.Max.x - 20 * Menu::DPI_SCALE - 25 * Menu::DPI_SCALE + Animation, total_bb.Min.y + (square_sz / 2)), 7 * Menu::DPI_SCALE, ImColor(255, 255, 255, (int)(ImGui::GetStyle().Alpha * 255)));

    ImVec2 label_pos = ImVec2(total_bb.Min.x + 15 * Menu::DPI_SCALE, total_bb.Min.y + (square_sz / 2) - CalcTextSize(label).y / 2);
    if (label_size.x > 0.0f)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    SetCursorPosY(GetCursorPosY() - 5 * Menu::DPI_SCALE);
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    IM_ASSERT((flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can't use both flags together
    const float size = ImGui::GetWindowWidth() - 15 * Menu::DPI_SCALE;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 43 * Menu::DPI_SCALE));
    const ImRect clickable(window->DC.CursorPos + ImVec2(13 * Menu::DPI_SCALE, 18 * Menu::DPI_SCALE), window->DC.CursorPos + ImVec2(size, 43 * Menu::DPI_SCALE));
    ...
    window->DrawList->AddRectFilled(clickable.Min, ImVec2(clickable.Max.x - 30 * Menu::DPI_SCALE, clickable.Max.y), ImColor(44, 44, 44, (int)(GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);

    window->DrawList->AddRect(clickable.Min, ImVec2(clickable.Max.x - 30 * Menu::DPI_SCALE, clickable.Max.y), ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);

    window->DrawList->AddText(ImVec2(clickable.Min.x + 1 * Menu::DPI_SCALE, rect.Min.y), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), label);

    window->DrawList->PushClipRect(clickable.Min, clickable.Max - ImVec2(31 * Menu::DPI_SCALE, 0));
    window->DrawList->AddText(clickable.Min + ImVec2(5 * Menu::DPI_SCALE, 12 * Menu::DPI_SCALE - CalcTextSize(preview_value).y / 2), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), preview_value);
    window->DrawList->PopClipRect();

    window->DrawList->AddRectFilled(ImVec2(clickable.Max.x - 25 * Menu::DPI_SCALE, clickable.Min.y), ImVec2(clickable.Max.x, clickable.Max.y), ImColor(44, 44, 44, (int)(GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);
    window->DrawList->AddRect(ImVec2(clickable.Max.x - 25 * Menu::DPI_SCALE, clickable.Min.y), ImVec2(clickable.Max.x, clickable.Max.y), ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), 3 * Menu::DPI_SCALE + 1);

    window->DrawList->AddCircleFilled(rect.Max - ImVec2(12 * Menu::DPI_SCALE, 13 * Menu::DPI_SCALE), (3.f + it_anim->second.active_circle_anim) * Menu::DPI_SCALE, ImColor(Menu::MenuColor.Value.x, Menu::MenuColor.Value.y, Menu::MenuColor.Value.z, GetStyle().Alpha));

    SetCursorPosY(GetCursorPosY() + 4 * Menu::DPI_SCALE);
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(0, 2 * Menu::DPI_SCALE));
        }

    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(5 * Menu::DPI_SCALE, 5 * Menu::DPI_SCALE)); // Horizontally align ourselves with the framed text
    PushStyleVar(ImGuiStyleVar_PopupBorderSize, 1.f);
    PushStyleVar(ImGuiStyleVar_PopupRounding, 3.f);
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(44, 44, 44, (int)(GetStyle().Alpha * 255))));
    PushStyleColor(ImGuiCol_Border, ImVec4(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255))));
...
static bool Items_ArrayGetterr(void* data, int idx, const char** out_text)
{
    const char* const* items = (const char* const*)data;
    if (out_text)
        *out_text = items[idx];
...
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetterr, (void*)items, items_count, height_in_items);
    return value_changed;
}
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT2(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderBehavior2(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent)
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
    ...
    switch (data_type)
    {
    case ImGuiDataType_S8: { ImS32 v32 = (ImS32) * (ImS8*)p_v;  bool r = SliderBehaviorT2<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min, *(const ImS8*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImS8*)p_v = (ImS8)v32;  return r; }
    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = SliderBehaviorT2<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min, *(const ImU8*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImU8*)p_v = (ImU8)v32;  return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32) * (ImS16*)p_v; bool r = SliderBehaviorT2<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = SliderBehaviorT2<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:
        IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
        return SliderBehaviorT2<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v, *(const ImS32*)p_min, *(const ImS32*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_U32:
        IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT2<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v, *(const ImU32*)p_min, *(const ImU32*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_S64:
        IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT2<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v, *(const ImS64*)p_min, *(const ImS64*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_U64:
        IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT2<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v, *(const ImU64*)p_min, *(const ImU64*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_Float:
        IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorT2<float, float, float >(bb, id, data_type, (float*)p_v, *(const float*)p_min, *(const float*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_Double:
        IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
        return SliderBehaviorT2<double, double, double>(bb, id, data_type, (double*)p_v, *(const double*)p_min, *(const double*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 5 * Menu::DPI_SCALE);
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(15 * Menu::DPI_SCALE, label_size.y + 2 * Menu::DPI_SCALE), window->DC.CursorPos + ImVec2(GetWindowSize().x - 15 * Menu::DPI_SCALE, 10 * Menu::DPI_SCALE + label_size.y + 2 * Menu::DPI_SCALE));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(GetWindowSize().x, 10 * Menu::DPI_SCALE + label_size.y + 2 * Menu::DPI_SCALE));
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior2(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb, &percent);
    if (value_changed)
        MarkItemEdited(id);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(44, 44, 44, (int)(ImGui::GetStyle().Alpha * 255)), 4 * Menu::DPI_SCALE + 1);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1 * Menu::DPI_SCALE, frame_bb.Min.y + 1 * Menu::DPI_SCALE), ImVec2(frame_bb.Min.x - 1 * Menu::DPI_SCALE + value->second, frame_bb.Max.y - 1 * Menu::DPI_SCALE), ImColor(Menu::MenuColor.Value.x, Menu::MenuColor.Value.y, Menu::MenuColor.Value.z, ImGui::GetStyle().Alpha), 4 * Menu::DPI_SCALE + 1);

    float animhv = FastFloatLerp(std::string(label).append("anif"), hovered, 0, 1, 0.04f);
    ImColor animationhover = FastColorLerp(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), ImColor(74, 74, 74, (int)(GetStyle().Alpha * 255)), animhv);

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, animationhover, 4 * Menu::DPI_SCALE + 1, ImDrawFlags_None, 1.5f);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + value->second, grab_bb.Min.y + 3 * Menu::DPI_SCALE), 7 * Menu::DPI_SCALE, ImColor(255, 255, 255, (int)(ImGui::GetStyle().Alpha * 255)));

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf, value_buf_end).x, total_bb.Min.y), ImColor(255, 255, 255, (int)(ImGui::GetStyle().Alpha * 255)), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x + 15 * Menu::DPI_SCALE, total_bb.Min.y), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGui::SetCursorPosY(GetCursorPosY() - 3 * Menu::DPI_SCALE);
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = ImVec2(ImGui::GetWindowSize().x - 30 * Menu::DPI_SCALE, 25 * Menu::DPI_SCALE);
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(13 * Menu::DPI_SCALE, label_size.y), window->DC.CursorPos + frame_size + ImVec2(15 * Menu::DPI_SCALE, label_size.y));
    const ImRect total_bb(frame_bb.Min - ImVec2(0, label_size.y), frame_bb.Min + total_size + ImVec2(0, 0));
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(44, 44, 44, (int)(GetStyle().Alpha * 255)), true, 3 * Menu::DPI_SCALE + 1);
        float animationhv = FastFloatLerp(std::string(label).append("fanifaw"), hovered, 0, 1, 0.04f);
        ImColor animcl = FastColorLerp(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), ImColor(64, 64, 64, (int)(GetStyle().Alpha * 255)), animationhv);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, animcl, 3 * Menu::DPI_SCALE + 1);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize + 1 * Menu::DPI_SCALE), rect_pos + ImVec2(rect_size.x, bg_offy_dn - 3 * Menu::DPI_SCALE));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, 2 * Menu::DPI_SCALE + 1);
                }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(frame_bb.Min + ImVec2(it_anim->second.value, 5 * Menu::DPI_SCALE), frame_bb.Min + ImVec2(it_anim->second.value, 20 * Menu::DPI_SCALE), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImU32 col = ImColor(186, 186, 186, (int)(GetStyle().Alpha * 255));
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + 2, total_bb.Min.y), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float square_sz = 18 * Menu::DPI_SCALE;
    const float w_full = ImGui::GetWindowSize().x;
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(15 * Menu::DPI_SCALE, 0);
    const float inputs_offset_x = (style.ColorButtonPosition == ImGuiDir_Left) ? w_button : 0.0f;
    window->DC.CursorPos.x = pos.x + w_full - square_sz - 30 * Menu::DPI_SCALE;
    static std::unordered_map<std::string, bool> valuesMapFloata;
    float divider = FastFloatLerp(std::string(label).append("animtAnimator"), valuesMapFloata.find(label)->second, 1, 10, 0.4f);
    auto valuezz = valuesMapFloata.find(label);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                if (valuezz == valuesMapFloata.end())
                {
                    valuesMapFloata.insert({ label, true });
                }
                else
                {
                    valuesMapFloata.find(label)->second = true;
                }
            }
        ...
        if (BeginPopup("picker"))
        {
            ...
            {
                ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImVec2(ImGui::GetWindowSize().x, 30), ImColor(23, 23, 23, (int)(GetStyle().Alpha * 255)), 5, ImDrawCornerFlags_Top);
                ImGui::GetWindowDrawList()->AddLine(ImGui::GetWindowPos() + ImVec2(0, 30), ImGui::GetWindowPos() + ImVec2(ImGui::GetWindowSize().x, 30), Menu::MenuColor);
                ImGui::GetWindowDrawList()->AddText(ImGui::GetWindowPos() + ImVec2(10, 15 - CalcTextSize("Color Picker").y / 2), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), "Color Picker");
                ...
                if (divider != 2)
                    SetNextItemWidth(square_sz * divider); // Use 256 + bar sizes?
                value_changed |= ColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.x);
                ImGui::SameLine();
                ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 10);

                ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (picker_flags & ImGuiColorEditFlags_AlphaBar ? 140 : 165));
            }
            EndPopup();
        }
        else
        {
            valuesMapFloata.find(label)->second = false;
        }
    }
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DrawList->AddText(ImVec2(pos.x, pos.y + (square_sz / 2) - CalcTextSize(label).y / 2), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), label);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bar0_pos_x = picker_pos.x + sv_picker_size + 5;
    float bar1_pos_x = bar0_pos_x + bars_width + 5;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size_arg)
{
    ...
    const float default_size = 18 * Menu::DPI_SCALE;
    ...
    float rounding = 3 * Menu::DPI_SCALE + 1;
    ImRect bb_inner = bb;
    float off = 0.0f;
    ...
    else
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(off, off), rounding);

        float animhv = FastFloatLerp(std::string(desc_id).append("anif"), hovered, 0, 1, 0.04f);
        ImColor animationhover = FastColorLerp(ImColor(54, 54, 54, (int)(GetStyle().Alpha * 255)), ImColor(74, 74, 74, (int)(GetStyle().Alpha * 255)), animhv);

        window->DrawList->AddRect(bb_inner.Min - ImVec2(1 * Menu::DPI_SCALE, 1 * Menu::DPI_SCALE), bb_inner.Max + ImVec2(1 * Menu::DPI_SCALE, 1 * Menu::DPI_SCALE), animationhover, rounding, 0, 1 * Menu::DPI_SCALE);
...
{
    const bool value_changed = ListBox(label, current_item, Items_ArrayGetterr, (void*)items, items_count, height_items);
    return value_changed;
}
